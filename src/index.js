...
const { runAgent, confirmPending: confirmV11Plan } = require("./core/agent");
...
client.once(Events.ClientReady, async () => {
  for (const guild of client.guilds.cache.values()) {
    try { const cfg=getConfig(guild.id); await serverKnowledge.scan(guild,cfg,saveConfig); } catch (e) { console.error('[KNOWLEDGE SCAN]',e); }
  }
});
...
function defaultConfig() {
  return {
    ...
    v12: { autopilot: { enabled:false }, snapshots:{ disabled:false, items:[] } },
    v13: { knowledge: {}, learning: { enabled:true, minObservations:3 } },
    v14: { reasoning: {}, autopilot: { correlation: true } },
    v9: {
      actionJournal: [],
...
function normalizeConfig(config) {
  ...
  config.v12 = {autopilot:{enabled:false},snapshots:{disabled:false,items:[]},...(config.v12||{})};
  config.v12.autopilot={enabled:false,...(config.v12.autopilot||{})};
  config.v13 ??= {}; config.v13.knowledge ??= {};
  config.v14 ??= {}; config.v14.reasoning ??= {}; config.v14.autopilot={correlation:true,...(config.v14.autopilot||{})}; config.v13.learning={enabled:true,minObservations:3,...(config.v13.learning||{})};
  config.v12.snapshots={disabled:false,items:[],...(config.v12.snapshots||{})};
  config.v9 ??= {};
...
registerCommand("mode", "System", async (message,args) => {
  ...
  if(!args[0]) return message.reply(`🎛️ **JARVIS V10 Mode:** **${cfg.ai?.personality||'classic'}**\nAvailable: ${modes.join(', ')}`);
...
registerCommand(["voice", "tts"], "System", async (message,args) => {
  if (!await requireAdmin(message)) return;
  ...
  if (sub==='leave') {
    voice.leave(guildId);
    delete message.guild.__jarvisVoiceConnection;
    return message.reply('🔊 Leaving the voice channel, sir.');
  }
  if (sub==='off' || sub==='disable') {
    voice.setTtsEnabled(guildId,false);
    return message.reply('🔇 TTS is now **OFF**, sir. JARVIS will continue replying in text.');
  }
  if (sub==='on' || sub==='enable') {
    if (!vc && !message.guild.__jarvisVoiceConnection) return message.reply('🔊 Join a voice channel first, sir.');
    if (vc && !message.guild.__jarvisVoiceConnection) {
      try { message.guild.__jarvisVoiceConnection=await voice.join(vc); }
      catch(e) { return message.reply(`❌ Voice startup failed: ${String(e.message||e).slice(0,500)}`); }
    }
    voice.setTtsEnabled(guildId,true);
    await voice.speakText(message.guild.__jarvisVoiceConnection,'TTS is online, sir.');
    return message.reply('🔊 TTS is now **ON**, sir.');
  }
  if (sub==='stop' || sub==='skip') {
    voice.stop(guildId,true);
    return message.reply('⏹️ TTS queue cleared, sir.');
  }
  if (sub==='pause') { voice.pause(guildId); return message.reply('⏸️ TTS paused, sir.'); }
  if (sub==='resume') { voice.resume(guildId); return message.reply('▶️ TTS resumed, sir.'); }
  if (sub==='status' || sub==='diagnose' || sub==='diagnostics') {
    const v=voice.status(guildId);
    return message.reply(`🩺 **JARVIS V10 TTS**\n• Voice deps: **${v.available?'ONLINE':'MISSING'}**\n• Connection: **${v.ready?'READY':v.connected?'CONNECTING':'OFFLINE'}**\n• TTS: **${v.ttsEnabled?'ON':'OFF'}**\n• Speaking: **${v.speaking?'YES':'NO'}**\n• Queue: **${v.queue}**\n• Model: **${v.model}**\n• Voice: **${v.voice}**\n• Accent: **${v.language}**`);
  }
  if (!vc) return message.reply('🔊 Join a voice channel first, sir.');
  if (!voice.status().available) return message.reply('🔊 Voice dependencies are missing. Run `npm install` and redeploy.');
  try {
    const connection=await voice.join(vc);
    message.guild.__jarvisVoiceConnection=connection;
    voice.setTtsEnabled(guildId,true);
    await voice.speakText(connection,'Voice systems online and TTS is ready, sir.');
    return message.reply(`🔊 Voice systems online in **${vc.name}**, sir. Text replies will now be spoken automatically.`);
  } catch(e) {
    console.error('[VOICE]',e);
    return message.reply(`❌ Voice startup failed: ${String(e.message||e).slice(0,500)}`);
  }
}, "Join voice and enable text-to-speech. No speech-to-text is used in V10.");
...
client.on(
  ...
  async message => {
    ...
    // invocation. No "jarvis" prefix is required. This is deliberately
    // limited to the owner and to a message whose referenced message was
    // actually sent by JARVIS, so random DMs cannot wake the AI pipeline.
    if (!message.guild) {
      const owner=getOwnerId();
      if (owner && message.author.id===owner && message.reference?.messageId) {
        const ref=await message.fetchReference().catch(()=>null);
        if (ref?.author?.id===client.user.id) {
          try {
            await message.channel.sendTyping().catch(()=>{});
            const reply=await conversationalReplyDM({message,prompt:rawContent});
            if(reply) await message.reply(reply.slice(0,1900));
          } catch(e) { console.error('[DM AI]',e); await message.reply('⚠️ My AI systems are temporarily unavailable, sir.').catch(()=>{}); }
        }
      }
      return;
    }
...
// ============================================================
// V13 SERVER KNOWLEDGE EVENTS
// ============================================================
client.on(Events.GuildRoleCreate, role => { try { const cfg=getConfig(role.guild.id); serverKnowledge.recordEvent(cfg,role.guild.id,{action:'ROLE_CREATE',detail:role.name}); saveConfig(role.guild.id,cfg); } catch {} });
client.on(Events.GuildRoleDelete, role => { try { const cfg=getConfig(role.guild.id); serverKnowledge.recordEvent(cfg,role.guild.id,{action:'ROLE_DELETE',detail:role.name}); saveConfig(role.guild.id,cfg); } catch {} });
client.on(Events.ChannelCreate, channel => { if(!channel.guild)return; try { const cfg=getConfig(channel.guild.id); serverKnowledge.recordEvent(cfg,channel.guild.id,{action:'CHANNEL_CREATE',detail:channel.name}); saveConfig(channel.guild.id,cfg); } catch {} });
client.on(Events.ChannelDelete, channel => { if(!channel.guild)return; try { const cfg=getConfig(channel.guild.id); serverKnowledge.recordEvent(cfg,channel.guild.id,{action:'CHANNEL_DELETE',detail:channel.name}); saveConfig(channel.guild.id,cfg); } catch {} });

// ============================================================
// V13.5 LEARNING EVENTS
// ============================================================
client.on(Events.GuildMemberAdd, member => { try { const cfg=getConfig(member.guild.id); serverKnowledge.recordEvent(cfg,member.guild.id,{action:'MEMBER_JOIN',detail:member.user.tag}); saveConfig(member.guild.id,cfg); } catch {} });
client.on(Events.GuildMemberRemove, member => { try { const cfg=getConfig(member.guild.id); serverKnowledge.recordEvent(cfg,member.guild.id,{action:'MEMBER_LEAVE',detail:member.user.tag}); saveConfig(member.guild.id,cfg); } catch {} });
client.on(Events.GuildRoleUpdate, (oldRole,newRole) => { try { const cfg=getConfig(newRole.guild.id); serverKnowledge.recordEvent(cfg,newRole.guild.id,{action:'ROLE_UPDATE',detail:newRole.name,permissionChanged:oldRole.permissions.bitfield.toString()!==newRole.permissions.bitfield.toString()}); saveConfig(newRole.guild.id,cfg); } catch {} });
client.on(Events.ChannelUpdate, (oldChannel,newChannel) => { if(!newChannel.guild)return; try { const cfg=getConfig(newChannel.guild.id); if(oldChannel.name!==newChannel.name)serverKnowledge.recordEvent(cfg,newChannel.guild.id,{action:'CHANNEL_RENAME',detail:`${oldChannel.name} → ${newChannel.name}`}); saveConfig(newChannel.guild.id,cfg); } catch {} });
