...
async function speakText(connection,text) {
  ...
  const ai=new GoogleGenAI({apiKey}); const model=process.env.GEMINI_TTS_MODEL||'gemini-2.5-flash-preview-tts'; const voiceName=process.env.JARVIS_TTS_VOICE||'Gacrux';
  const response=await ai.models.generateContent({model,contents:[{parts:[{text:`# AUDIO PROFILE: JARVIS\nA refined British male AI butler with a calm, intelligent, understated presence.\n\n# DIRECTOR'S NOTES\nAccent: refined modern British English, natural and subtle, not exaggerated.\nTone: composed, confident, warm, precise, slightly formal.\nPace: measured and conversational, with clean articulation and brief natural pauses.\nDelivery: sophisticated and restrained, like a discreet high-end personal assistant. Never imitate or reproduce any specific actor or fictional performance.\n\n# TRANSCRIPT\n${String(text).slice(0,3000)}`}]}],config:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName}}}}});
