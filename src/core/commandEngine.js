...
function pickFromCandidates(text,candidates){
  ...
  if(numMatch){const idx=Number(numMatch[1])-1;if(candidates[idx])return candidates[idx];}
  const theOrd=raw.match(/^(?:the\s+)?(\w+)$/i);
  if(theOrd){const idx=ORDINALS.indexOf(theOrd[1].toLowerCase());if(idx>=0&&candidates[idx])return candidates[idx];}
  const norm=raw.toLowerCase().replace(/^[@#]/,'');
  let best=null,bestScore=-1;
  for(const cand of candidates){
    const names=[cand.name,cand.displayName,cand.user?.tag,cand.user?.username].filter(Boolean).map(n=>n.toLowerCase());
    for(const n of names){
      let score=-1;
      if(n===norm)score=100;else if(n.startsWith(norm))score=80;else if(n.includes(norm))score=50;
...
async function route({message,text,config,saveConfig}){
  ...
  if(c){
    if(/^(?:cancel|no)$/i.test(text)){context.clear(message);return {handled:true,text:'Cancelled, sir.'};}
...
async function perform({message,intent,config,saveConfig,confirmed=false}){
  ...
  const targetResult=await resolveTargets(message,intent.targets||[]);if(targetResult.error){context.set(message,{intent,candidates:targetResult.candidates||[],destination:intent.destination,kind:'member'});return {handled:true,text:targetResult.error};}
