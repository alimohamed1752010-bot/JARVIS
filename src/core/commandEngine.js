...
const {creatorAnswer}=require('./identity');
const toolRegistry=createDefaultRegistry();

const ACTION_WORDS=/\b(move|drag|send|disconnect|dc|pull|deafen|undeafen|mute|unmute|timeout|untimeout|kick|ban|warn|simulate|undo|history|diagnostics|status|who|everyone|except|role|roles|permission|permissions|soundboard|server|channel|nickname|rename|give|remove|add)\b/i;
...
function normalizeAction(a){return ({move_voice:'voicemove',move:'voicemove',disconnect:'voicedisconnect',deafen:'voicedeafen',undeafen:'voiceundeafen',server_mute:'voicemute',server_unmute:'voiceunmute',timeout:'timeout',untimeout:'untimeout',kick:'kick',ban:'ban',role_permissions:'role_permissions',role_add:'role_add',role_remove:'role_remove',channel_edit:'channel_edit'})[String(a||'').toLowerCase()]||String(a||'').toLowerCase();}
function deterministic(text){
  const t=String(text).trim();
  let m=t.match(/^(?:move|drag|send)\s+(everyone|everybody|all)\s+in\s+(.+?)\s+(?:to|into)\s+(.+?)(?:\s+except\s+(.+))?$/i);
  if(m)return {action:'voicemove',targets:['everyone'],excludeTargets:m[4]?splitTargets(m[4]):[],source:m[2].trim(),destination:m[3].trim()};
  m=t.match(/^(?:move|drag|send)\s+(.+?)\s+(?:from\s+(.+?)\s+)?(?:to|into)\s+(.+)$/i);
  ...
  m=t.match(/^(?:change|edit|update|set)\s+(?:the\s+)?(?:<@&(\d+)>|@?(.+?))\s+(?:role\s+)?(?:permissions?|perms?)\s+(?:to\s+)?(.+)$/i);
  if(m){
    const role=m[1]?`<@&${m[1]}>`:m[2].trim();
    const changes=[];
    for(const part of m[3].split(/\s*(?:,|and)\s*/i)){
      const x=part.trim(); if(!x) continue;
      const off=x.match(/^(?:remove|deny|disable|take away|revoke)\s+(.+)$/i);
      const on=x.match(/^(?:add|allow|enable|grant|give)\s+(.+)$/i);
      if(off) changes.push({permission:off[1].trim(),enabled:false});
      else if(on) changes.push({permission:on[1].trim(),enabled:true});
    }
    if(changes.length)return {action:'role_permissions',role,permissionChanges:changes};
  }
  m=t.match(/^(?:give|add)\s+(?:the\s+)?role\s+(.+?)\s+(?:to|on)\s+(.+)$/i);if(m)return {action:'role_add',role:m[1].trim(),targets:splitTargets(m[2])};
  m=t.match(/^(?:remove|take)\s+(?:the\s+)?role\s+(.+?)\s+(?:from)\s+(.+)$/i);if(m)return {action:'role_remove',role:m[1].trim(),targets:splitTargets(m[2])};
  m=t.match(/^(?:rename|change)\s+(?:the\s+)?(?:channel|room)\s+(.+?)\s+(?:to|as)\s+(.+)$/i);if(m)return {action:'channel_edit',channel:m[1].trim(),name:m[2].trim()};
...
async function perform({message,intent,config,saveConfig,confirmed=false}){
  if(['unknown',''].includes(normalizeAction(intent.action)))return {handled:false};
  if(intent.action==='jarvis_identity'){return {handled:true,text:creatorAnswer()};}
  ...
  if(intent.action==='diagnostics'){const snap=await awareness.snapshot(message.guild);return {handled:true,text:`**JARVIS V11.6.9 DIAGNOSTICS**\n🟢 Discord ONLINE\n🟢 Command Engine ONLINE\n🟢 Resolver ONLINE\n🟢 Executor ONLINE\n🟢 Journal ONLINE\n🟢 Context ONLINE\n🟢 Tool Registry ONLINE (${toolRegistry.list().length} core tools)\n\n${awareness.format(snap)}`};}
  ...
  if(intent.action==='history'){let ref=intent.targets?.[0];if(ref&&!/^\d{15,25}$/.test(ref)){const r=await resolveMember(message.guild,ref);if(r.status==='resolved')ref=r.member.id;}const entries=ref?(config.v9?.actionJournal||[]).filter(x=>x.targetId===ref).slice(-10).reverse():(config.v9?.actionJournal||[]).slice(-10).reverse();return {handled:true,text:`**JARVIS V11 ACTION HISTORY**\n${entries.map(x=>`#${x.id} • ${x.action} • <@${x.targetId||message.author.id}> • ${x.status} • ${new Date(x.at).toLocaleString()}`).join('\n')||'No actions logged yet.'}`};}
  if(intent.action==='role_permissions' && !confirmed){confirmations.create(message,{intent});return {handled:true,text:`This will change permissions for **${intent.role||'the specified role'}**. Reply **yes** to proceed or **no** to cancel.`};}
  if(['role_permissions','role_add','role_remove','channel_edit'].includes(intent.action)){
    const result=await execute({message,action:intent.action,target:null,destination:null,reason:intent.reason,config,saveConfig,role:intent.role,permissionChanges:intent.permissionChanges,targets:intent.targets,channel:intent.channel,name:intent.name});
