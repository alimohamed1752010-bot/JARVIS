...
function getAIStatus(){ const apiKey=String(process.env.GEMINI_API_KEY||'').trim(); const enabled=String(process.env.AI_ENABLED??'true').toLowerCase()!=='false'; const model=String(process.env.GEMINI_MODEL||'gemini-3.5-flash-lite').trim(); return {enabled,configured:Boolean(apiKey),model,keyFormat:apiKey?'configured':'missing'}; }
