...
async function parseAgentPlan({message,prompt}) {
  const status=getAIStatus();
  if(!status.enabled || !status.configured) return null;
  const instruction=`You are JARVIS V18.2 UNIFIED PC + DISCORD AGENT PLANNER. Understand the user's intent FIRST, then convert it into a safe JSON execution plan. Never echo a generic conversational response when the user clearly asked for a PC or Discord action. Convert casual wording, shorthand, punctuation, and multi-step sentences into concrete ordered actions. The request may target Discord, Windows PC, files, apps, browser, keyboard, mouse-like input, screenshots, or multiple environments. NEVER execute anything. Return JSON only.
Schema: {"summary":"short summary","needsConfirmation":false,"steps":[{"action":"pc_open_app|pc_close_app|pc_processes|pc_system_status|pc_active_window|pc_network_status|pc_state|pc_volume|pc_key|pc_mouse|pc_hotkey|pc_type|pc_screenshot|pc_open_url|pc_browser_search|pc_spotify_play|pc_shell|pc_file_read|pc_file_write|pc_file_copy|pc_file_move|pc_file_delete|voicemove|voicedisconnect|voicemute|voiceunmute|voicedeafen|voiceundeafen|textmute|textunmute|timeout|untimeout|kick|ban|warn|role_permissions|role_add|role_remove|role_edit|channel_edit|channel_create|channel_delete|role_create|role_delete|member_nickname|channel_permissions|server_analyze|server_relationship|server_investigate|server_snapshot|server_audit|server_restore|server_diff|undo|autopilot","targets":[],"excludeTargets":[],"source":"","destination":"","role":"","channel":"","parent":"","channelType":"text","name":"","color":"","hoist":false,"mentionable":false,"permissionChanges":[],"reason":"","durationMs":600000}]}
Rules:
