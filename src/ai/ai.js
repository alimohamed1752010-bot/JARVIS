...
async function parseAgentPlan({message,prompt}) {
  const status=getAIStatus();
  if(!status.enabled || !status.configured) return null;
  const instruction=`You are JARVIS V16 UNIFIED PC + DISCORD AGENT PLANNER. Convert the user's natural-language request into a safe JSON execution plan. The request may target Discord, Windows PC, files, apps, browser, keyboard, mouse-like input, screenshots, or multiple environments. NEVER execute anything. Return JSON only.
Schema: {"summary":"short summary","needsConfirmation":false,"steps":[{"action":"pc_open_app|pc_close_app|pc_processes|pc_volume|pc_key|pc_mouse|pc_hotkey|pc_type|pc_screenshot|pc_open_url|pc_browser_search|pc_spotify_play|pc_shell|pc_file_read|pc_file_write|pc_file_copy|pc_file_move|pc_file_delete|voicemove|voicedisconnect|voicemute|voiceunmute|voicedeafen|voiceundeafen|textmute|textunmute|timeout|untimeout|kick|ban|warn|role_permissions|role_add|role_remove|role_edit|channel_edit|channel_create|channel_delete|role_create|role_delete|member_nickname|channel_permissions|server_analyze|server_relationship|server_investigate|server_snapshot|server_audit|server_restore|server_diff|undo|autopilot","targets":[],"excludeTargets":[],"source":"","destination":"","role":"","channel":"","parent":"","channelType":"text","name":"","color":"","hoist":false,"mentionable":false,"permissionChanges":[],"reason":"","durationMs":600000}]}
Rules:
- Understand casual natural language, shorthand, typos, and multi-step requests.
- PC actions: pc_open_app uses name=application name and targets as optional app arguments/URLs; pc_browser_search uses name=the YouTube search query and reason=browser name (default brave); pc_spotify_play uses name=the track/search query and opens Spotify then selects the first search result; pc_open_app uses name=application name; pc_close_app uses name=application name; pc_processes is read-only; pc_volume uses durationMs as a 0-100 volume value; pc_key and pc_hotkey use name for the key sequence; pc_mouse uses name='x,y[,left|right|middle|double]'; pc_type uses reason for the exact text; pc_screenshot optionally uses name for the output path; pc_open_url uses name for the exact URL; pc_shell uses reason for the exact PowerShell command; PC file actions use name=source path and reason=destination/content where applicable.
...
- A request can contain both Discord and PC steps. Preserve the requested order.
